...
# --- Add Gaussian emission line(s) ---
def add_emission_line(wav_obs, total_flux, line_center_rest, 
                      line_luminosity, redshift, sigma_A=10.0): #ashiq to output wave_obs?, line_center_rest, line_luminosity, (sigma once we figure that out)
    """
    wav_obs: observed wavelength array (micron)
    total_flux: spectrum array to modify (uJy)
    line_center_rest: rest wavelength (Å)
    line_luminosity: intrinsic line luminosity (erg/s)
    redshift: redshift of the object
    sigma_A: line width (Å)
    """

    # Convert rest λ to observed (micron)
    line_center_obs = line_center_rest * (1 + redshift) * 1e-4  # Å→µm
    sigma_obs = sigma_A * (1 + redshift) * 1e-4

    # Compute amplitude in flux units (same conversion as continuum) (uses luminosity distance to find flux on a portion of a sphere given a luminous source)
    flux_norm = line_luminosity / (4*np.pi*d_L0**2)  # erg/s/cm^2 ###is this how I am inputing this data

    # Convert to uJy
    amplitude_uJy = flux_norm * 1e29

    # Gaussian profile
    profile = amplitude_uJy * np.exp(-0.5 * ((wav_obs-line_center_obs)/sigma_obs)**2)
...
ax1.plot(z1,massc1,label='CC')
ax1.plot(z2,massc2,label='low')
ax1.plot(z3,massc3,label='high')
ax1.set_xlim(13,6)
plt.legend()
